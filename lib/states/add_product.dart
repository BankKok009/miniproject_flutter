...
class _AddProductState extends State<AddProduct> {
        ...
        for (var item in files) {
          ...
          await Dio().post(apiSaveProduct, data: data).then((value) async {
            ...
            if (loop >= files.length) {
              ...
              print('### idSerler = $idSeller, nameSeller = $nameSeller');
              print('### name = $name, price = $price, detail = $detail');
              print('### images ==> ${images}');

              String path =
                  '${MyConstant.domain}/miniprojectmoblie/InsertProduct.php?isAdd=true&idSeller=$idSeller&nameSeller=$nameSeller&name=$name&price=$price&detail=$detail&images=$images';
  ...
  Future<Null> proacessImagePicker(ImageSource source, int index) async {
    try {
          ...
          Row(
            ...
            children: [
              TextButton(
                onPressed: () {
                  Navigator.pop(context);
                  proacessImagePicker(ImageSource.camera, index);
                },
                child: Text('Camera'),
              ),
              TextButton(
                onPressed: () {
                  Navigator.pop(context);
                  proacessImagePicker(ImageSource.gallery, index);
                },
                child: Text('Gallery'),
              ),
            ],
    ...
    return Column(
      children: [
        Container(
          ...
          child: file == null
              ? Image.asset(MyConstant.imageIcon)
              : Image.file(file!),
        ),
        Container(
          width: constraints.maxWidth * 0.75,
